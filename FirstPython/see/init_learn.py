...
#top = tkinter.Tk();
#top.mainloop(0);
#response = urllib.request.urlopen(("http://www.msn.com"));
#print (response.read());
...
def query21():
    ...
    for e in links:
        if e.submitter_id == 62443:
            result.append((e.submitted_time, e));
    return [l for t, l in sorted(result, key=itemgetter(0))];
...
def query2():
    ...
    for e in links:
        if e.submitter_id == 62443:
            result.append(e);
    result.sort(key=lambda k: k.submitted_time);
    return result;
