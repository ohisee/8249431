...
def addresses1(haystack):
    rgex = r'[A-Za-z]+\@[A-Za-z]+(?:\.[A-Za-z]+)+';
    emails = re.findall(rgex, haystack);
    return [email.replace('NOSPAM', '') for email in emails];
...
def addresses(haystack):
    emaillexer = lex.lex();
    emaillexer.input(haystack);
    result = []; 
    while True:
        tok = emaillexer.token();
        if not tok: break;
