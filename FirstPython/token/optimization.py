...
def eval_binary_exp(exp):
    etype = exp[0];
    if etype == "binop":
        left_child = exp[1]
        right_child = exp[3]
        left_value = eval_binary_exp(left_child);
        right_value = eval_binary_exp(right_child);
...
# ("assign", variable_name, rhs_expression) 
#
def lookup_env_exp(exp, op_env):
    for i in range(len(op_env)):
        if exp == op_env[i][2]:
            return i;
    return None;

def lookup_env_variable(variable, op_env):
    for i in range(len(op_env)):
        if variable == op_env[i][1]:
            return i;
    return None;
...
def optimize(ast):
    ...
    for i in range(len(ast)):
        exp = ast[i];
        if exp[0] == "assign":
            vk = lookup_env_exp(exp[2], assign_op_env); # first look up
            assign_op_env.append(ast[i]);               # second add into env
            if vk is not None:
                vs = eval_binary_exp(exp[2]);
                #assign_op_env.append(ast[i]);
                if not any([True if lookup_env_variable(v, assign_op_env[vk:]) is not None else False for v in vs]):
                    ast[i] = (exp[0], exp[1], ("identifier", assign_op_env[vk][1]));
                    assign_op_env.pop(vk);
            #else:
                #assign_op_env.append(ast[i]);
    return ast;
...
#bin_a = {'w' : ('number', 90)}
#vs = eval_binary_exp( ('binop', ('binop', ('identifier', 'a'), '+', ('identifier', 'b')), '+', ('identifier', 'c')));
#print any([True if v in bin_a else False for v in vs]);
