...
def filter_maker(f):
    #return lambda lst : [n for n in lst if f(n) == True];
    def apply_filter_function (lst):
        return [n for n in lst if f(n) == True];
    return apply_filter_function;

def map_maker(f):
    #return lambda lst : [f(n) for n in lst];
    def apply_map_maker (lst):
        return [f(n) for n in lst];
    return apply_map_maker;
