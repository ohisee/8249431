 ...
 *
 */
public class WeatherDataParser extends AndroidTestCase {
	...
	public double getMaxTemperatureForDay(String weatherJsonStr, int dayIndex)
		...
		JSONObject jsonDay = jArray.getJSONObject(dayIndex);
		JSONObject jsonTemperature = jsonDay.getJSONObject("temp");
		// TODO: add parsing code here
		return jsonTemperature.getDouble("max");
	}

	public void testGetMaxTemperatureForDay() {
		try {
			double t = getMaxTemperatureForDay(WEATHER_DATA_FREMONT_JUN_4, 2);
			double expected = 27.55;
			assertTrue(t == expected);
			
			t = getMaxTemperatureForDay(WEATHER_DATA_MTV_JUN_4, 2);
			expected = 14.1;
			assertTrue(t == expected);
		} catch (JSONException ignored) {}
	}
	
	public void testUtilityGetFormattedMonthDay() {
		String inputDateStr = "20141105";
		String result = Utility.getFormattedMonthDay(mContext, inputDateStr);
		assertTrue(result != null);
	}
	
	public void testUtilityGetFriendlyDayString() {
		String inputDateStr = "20141105";
		String result = Utility.getFriendlyDayString(mContext, inputDateStr);
		assertTrue(result != null);
	}
