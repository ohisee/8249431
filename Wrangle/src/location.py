...
def check_loc(point, lat, longi):
    # YOUR CODE HERE
    if is_float(lat) and is_float(longi):
        if point and (not point.startswith("{")) and len(re.split(r'\s+', point)) == 2:
            pointlat, pointlong = re.split(r'\s+', point);
