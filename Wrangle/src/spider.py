    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for process_field in process_fields:
                if 'rdf-schema#label' == process_field:
                    label = re.sub(r'\(.*\)', '', line[process_field], count = 1);
                    row[FIELDS[process_field]] = label.strip();
                if 'name' == process_field:
                    name = line[process_field].strip();
                    if name == 'NULL' or re.search(r'\W+', name) is not None:
                        name = label;
                    row[FIELDS[process_field]] = name.strip();
                if 'synonym' == process_field:
                    synonym = line[process_field].strip();
                    if synonym != 'NULL':
                        synonym = parse_array(re.sub(r'\**', '', synonym));
                    else:
                        synonym = None;
                    row[FIELDS[process_field]] = synonym;
                if process_field in ['family_label', 'class_label', 'phylum_label', 'order_label', 'kingdom_label', 'genus_label']:
                    if line[process_field] == 'NULL':
                        fi = None;
                    else:
                        fi = line[process_field].strip();
                    classification[FIELDS[process_field]] = fi;
                    row['classification'] = classification;
                if 'URI' == process_field or 'rdf-schema#comment' == process_field:
                    row[FIELDS[process_field]] = line[process_field].strip();
